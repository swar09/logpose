...
use diesel::{BoolExpressionMethods, ExpressionMethods, PgConnection, QueryDsl, RunQueryDsl, SelectableHelper};
use uuid::Uuid;

use crate::{
    models::url::{NewUrl, UpdateCode, UpdateUrl, Urls},
    schema::urls::{self, created_by, database_id, short_code},
};
...
pub fn delete_by_short_code(code: String, conn: &mut PgConnection) -> Result<usize, diesel::result::Error> {
...
pub fn modify_url_by_id(
    ...
    diesel::update(urls::table.find(id)).set(new_long_url).execute(conn)
}
...
pub fn modify_code_by_id(
    ...
    diesel::update(urls::table.find(id)).set(new_short_code).execute(conn)
}
...
pub fn get_by_short_code(code: String, conn: &mut PgConnection) -> Result<Urls, diesel::result::Error> {
    urls::table
...
pub fn get_long_url_by_id(id: i32, conn: &mut PgConnection) -> Result<String, diesel::result::Error> {
    urls::table
        .filter(database_id.eq(id))
...
pub fn count_urls_by_user_id(user_id_val: Uuid, conn: &mut PgConnection) -> Result<i64, diesel::result::Error> {
    urls::table
...
pub fn check_short_code_exists(code: &str, conn: &mut PgConnection) -> Result<bool, diesel::result::Error> {
    use diesel::{dsl::exists, select};
    select(exists(urls::table.filter(short_code.eq(code)))).get_result(conn)
}

pub fn get_user_id_by_short_code(code: String, conn: &mut PgConnection) -> Result<Option<Uuid>, diesel::result::Error> {
    urls::table.filter(short_code.eq(code)).select(created_by).first(conn)
}

pub fn get_long_url_by_short_code(code: String, conn: &mut PgConnection) -> Result<String, diesel::result::Error> {
    urls::table
        .filter(short_code.eq(code))
        .filter(urls::expires_at.is_null().or(urls::expires_at.gt(diesel::dsl::now)))
        .select(urls::long_url)
